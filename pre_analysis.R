...
filtered_data <- filtered_data %>%
  ...
  left_join(
    pop_65_plus %>% select(country, date, Population_65_plus_percent),
    by = c("country", "date")
  )
...
# Rename certain columns more succinctly (optional):
filtered_data <- filtered_data %>%
  rename(
    Universal_Health_Coverage_Index = `universal-health-coverage-index__uhc_service_coverage_index__sdg_3_8_1`,
    Child_mortality_rate = `child-mortality-igme__obs_value__indicator_under_five_mortality_rate__sex_total__wealth_quintile_total__unit_of_measure_deaths_per_100_live_births`,
    Life_expectancy_female = `life-expectancy-of-women-vs-life-expectancy-of-men__life_expectancy__sex_female__age_0__variant_estimates`,
    Life_expectancy_male = `life-expectancy-of-women-vs-life-expectancy-of-men__life_expectancy__sex_male__age_0__variant_estimates`,
    Life_expectancy_both = `life-expectancy-of-women-vs-life-expectancy-of-men__population__sex_all__age_all__variant_estimates`,
    Gini_coefficient_female_lifespan = `gini-coefficient-of-lifespan-inequality-in-females__life_expectancy_gini__sex_female`,
    Gini_coefficient_male_lifespan = `gini-coefficient-of-lifespan-inequality-in-males__life_expectancy_gini__sex_male`,
    Multidimensional_Poverty_Index_MPI = `multidimensional-poverty-index-mpi__mpi__area_national__flavor_current_margin_estimate`,
    Share_no_formal_education = `share-of-population-15-years-and-older-with-no-education__mf_youth_and_adults__15_64_years__percentage_of_no_education`,
    Mean_years_schooling = `mean-years-of-schooling-long-run__mf_youth_and_adults__15_64_years__average_years_of_education`,
    Self_reported_life_satisfaction = `happiness-cantril-ladder__cantril_ladder_score`,
    Share_happy_people = `share-of-people-who-say-they-are-happy__happy`,
    Atkinson_index_income_inequality = `income-inequality-atkinson-index-undp__ineq_inc`,
    Human_rights_index = `human-rights-index-vdem__civ_libs_vdem__estimate_best__aggregate_method_average`,
    Private_civil_liberties_index = `private-civil-liberties-index__priv_libs_vdem__estimate_best__aggregate_method_average`,
    LGBT_legal_equality_index = `lgbt-legal-equality-index__ei_legal`,
    Territory_controlled_by_gov_percent = `percentage-of-territory-controlled-by-government__terr_contr_vdem__estimate_best__aggregate_method_average`,
    Impartial_public_administration_index = `rigorous-and-impartial-public-administration-index__public_admin_vdem__estimate_best__aggregate_method_average`,
    State_capacity_index = `state-capacity-index__capacity`,
    Functioning_gov_index = `functioning-government-index-eiu__funct_gov_eiu`,
    Political_corruption_index = `political-corruption-index__corruption_vdem__estimate_best__aggregate_method_average`,
    Homicide_rate = `homicide-rate-unodc__value__category_total__sex_total__age_total__unit_of_measurement_rate_per_100_000_population`,
    Share_extreme_poverty = `Share-of-population-living-in-extreme-poverty`,
    Corruption_Perception_Index = `Corruption Perception Index`,
    DALYs_all_causes = `Disability-Adjusted Life Years (DALYs) per 100,000 individuals from all causes`
  )


###############################################################################
# 17. GROUP AND SUMMARIZE BY REGION & YEAR
###############################################################################
...
indicators_mean <- c(
  "Gini index",
  "Self_reported_life_satisfaction",
  "Atkinson_index_income_inequality",
  "Human_rights_index",
  "Private_civil_liberties_index",
  "LGBT_legal_equality_index",
  "Impartial_public_administration_index",
  "State_capacity_index",
  "Functioning_gov_index",
  "Political_corruption_index"
)

# Indicators weighted by land area
indicators_weighted_area <- c(
  "Territory_controlled_by_gov_percent"
)

# Summarize by region & year
df_region_year <- filtered_data %>%
  ungroup() %>%
  group_by(region, date) %>%
  summarise(
    publications = n(),
    citations = sum(times_cited, na.rm = TRUE),
    h_index_median = median_min(h_index),
    
    # Weighted by total population
    across(
      all_of(indicators_weighted),
      ~ weighted_mean_min(.x, Population_total),
      .names = "{.col}"
    ),
    ...
    across(
      ...
      ~ weighted_mean_min(.x, Land_area_sqkm),
      .names = "{.col}"
    ),
    ...
    # Normalized indicators (ratio of sums)
    Child_mortality_rate = normalize_indicator(
      Child_mortality_rate, Birth_rate_crude_per_1000
    ) * 1000,
    
    Gini_coefficient_female_lifespan = normalize_indicator(
      Gini_coefficient_female_lifespan,
      Death_rate_crude_per_1000
    ),
    
    Gini_coefficient_male_lifespan = normalize_indicator(
      Gini_coefficient_male_lifespan,
      Death_rate_crude_per_1000
    ),
    
    Multidimensional_Poverty_Index_MPI = normalize_indicator(
      Multidimensional_Poverty_Index_MPI,
      `Multidim_poverty_headcount_ratio_WB_percent`
    ),
    
    Share_no_formal_education = normalize_indicator(
      Share_no_formal_education,
      Population_15_64_total
    ),
...
###############################################################################
# 18. GROUP AND SUMMARIZE BY INCOME LEVEL & YEAR
###############################################################################
df_income_year <- filtered_data %>%
  ungroup() %>%
